...
export default function CreateListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = { ...Object.fromEntries(formData), price: Number(formData.get('price')), lookingFor, occupancy, roomType, mobileVis, highlights: selectedHighlights, amenities: selectedAmenities }
    try {
      const res = await fetch('http://localhost:8000/api/listings/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
